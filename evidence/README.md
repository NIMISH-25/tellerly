...
| `replay-20260827T231938Z-ec63cd` | Same restricted member with `--escalate`: intervention raised and handled by a **live human operator** at the terminal console (`look`, notes, `done abort`) | HARD_FAILURE `aborted_by_operator`, 1 InterventionRecord carrying the operator's own notes ("verified with supervisor — the restriction on 55555 is legitimate"), intervention screenshot + redacted DOM snapshot, full control-token history in events | 0 | $0.00 | ~3s |
