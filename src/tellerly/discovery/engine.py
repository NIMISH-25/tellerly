        ...
        return "\n".join(
            [
                ...
                *output_lines,
                ...
                "  input values: assert only text that appears for EVERY record.",
                "  GOOD: page headings and labels ('Member Record', 'TRANSFER",
                "  POSTED') or {{input.name}} placeholders. BAD (breaks every",
                "  other replay): a person's name, an account balance, a date, or",
                "  any value you just read. When in doubt, assert the heading.",
